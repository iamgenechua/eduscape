...
public class AudioManager : MonoBehaviour {
    ...
    public class AudioManagersMerger : MonoBehaviour {
        ...
        private static void MergeAudioManagers() {
            ...
            // merge sound FX lists
            SoundFx[] mergedSfx = new SoundFx[sfxList.Count + audioManagerInScene.soundFxs.Length];
            sfxList.ToArray().CopyTo(mergedSfx, 0);
            audioManagerInScene.soundFxs.CopyTo(mergedSfx, sfxList.Count);
            audioManagerInScene.soundFxs = mergedSfx;

            // merge music lists
            Music[] mergedMusic = new Music[audioManagerInScene.musicTracks.Length + musicList.Count];
            musicList.ToArray().CopyTo(mergedMusic, 0);
            audioManagerInScene.musicTracks.CopyTo(mergedMusic, musicList.Count);
            audioManagerInScene.musicTracks = mergedMusic;
        }
    }
}
